...
import 'package:selene/core/theme/theme_data.dart';
...
class ThemesDao extends DatabaseAccessor<AppDatabase> with _$ThemesDaoMixin {
  ...
  Stream<List<AppTheme>> watchAllThemes() {
    return select(themes).watch().map((rows) {
      ...
      return rows.map((row) => _mapRowToAppTheme(row)).toList();
    });
  }
  ...
  Future<AppTheme?> getThemeById(String id) async {
    ...
    return row != null ? _mapRowToAppTheme(row) : null;
  }
  ...
  Future<void> upsertTheme(AppTheme theme) {
    ...
    final companion = _mapAppThemeToCompanion(theme);
  ...
  AppTheme _mapRowToAppTheme(Theme row) {
    return AppTheme(
      id: row.id,
      name: row.name,
  ...
  ThemesCompanion _mapAppThemeToCompanion(AppTheme theme) {
